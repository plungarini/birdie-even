	...
	unsubscribeEvenHubEvent = b.onEvenHubEvent((event) => {
		...
		if (!firstAudioEventLogged && (event as { audioEvent?: unknown }).audioEvent !== undefined) {
			console.log(
				...
				JSON.stringify(Object.keys((event as unknown as { audioEvent: Record<string, unknown> }).audioEvent)),
			);
			firstAudioEventLogged = true;
		}

		// Both glasses and phone mic audio now arrive through the bridge's
		// native audio pipeline (SDK >= 0.0.11), so we no longer gate on
		// activeSource here - only on whether a capture session is active.
		if (!active) return;
...
export async function startCapture(): Promise<void> {
	...
	try {
		const source = activeSource === 'phone' ? AudioInputSource.Phone : AudioInputSource.Glasses;
		await bridge!.audioControl(true, source);
...
export async function stopCapture(): Promise<void> {
	...
	try {
		...
		console.error('[capture] stopCapture failed', { err });
	}
}
